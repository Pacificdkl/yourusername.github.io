...
| High | **PostgreSQL adapter + RLS** | **DONE** — all tables migrated; `PgStore` implements the full store (`tests/db/pg-store.test.ts`, every method vs pglite) with `SET LOCAL app.current_user` for RLS-scoped ops and transactional cascades; `tests/db/pg-schema.test.ts` proves RLS blocks cross-user reads and the §6 constraints hold. Production wiring done (`pg.Pool`→`Db`, store selected by `DATABASE_URL`, in-memory refused at prod runtime; `db:migrate` runner) — ADR 0015. **Operational**: connect as a non-`BYPASSRLS` role. |
